...
export class ImageCropperComponent implements OnChanges, OnInit {
    ...
    private setMaxSize(): void {
        if (this.sourceImage) {
            ...
            this.cropperStaticWidth = this.maxSize.width - 100;
            this.cropperStaticHeight = this.maxSize.height - 100;
    ...
    private checkCropperPosition(maintainSize = false): void {
        const scale = this.transform.scale ? this.transform.scale : 1;
        const outLine = {
            x1: -(this.maxSize.width * scale - this.cropperStaticWidth) / 2 + 50,
            y1: -(this.maxSize.height * scale - this.cropperStaticHeight) / 2 + 50,
            x2: (this.maxSize.width * scale + this.cropperStaticWidth) / 2 - 50,
            y2: (this.maxSize.height * scale + this.cropperStaticHeight) / 2 - 50,
        }
        if (this.cropper.x1 < outLine.x1) {
            this.cropper.x2 = maintainSize ? this.cropperStaticWidth : this.cropper.x2;
            this.cropper.x1 = outLine.x1;
        }
        if (this.cropper.y1 < outLine.y1) {
            this.cropper.y2 = maintainSize ? this.cropperStaticHeight : this.cropper.y2;
            this.cropper.y1 = outLine.y1;
        }
        if (this.cropper.x2 > outLine.x2) {
            this.cropper.x1 = maintainSize ? (outLine.x2 - this.cropperStaticWidth) : this.cropper.x1;
            this.cropper.x2 = outLine.x2;
        }
        if (this.cropper.y2 > outLine.y2) {
            this.cropper.y1 = maintainSize ? (outLine.y2 - this.cropperStaticHeight) : this.cropper.y1;
            this.cropper.y2 = outLine.y2;
        }
    }
    ...
    private move(event: any) {
        ...
        this.cropper.x2 = this.cropper.x1 + this.cropperStaticWidth;
        this.cropper.y2 = this.cropper.y1 + this.cropperStaticHeight;
    }
    ...
    crop(): ImageCroppedEvent | null {
        if (this.sourceImage && this.sourceImage.nativeElement && this.transformedImage != null) {
            ...
            if (ctx) {
                ...
                ctx.setTransform(scaleX, 0, 0, scaleY, this.cropper.x1*ratio-50*ratio*scaleX + width/ 2, this.cropper.y1*ratio-50*ratio*scaleY + height/ 2);
                ctx.drawImage(this.transformedImage, -width/ 2, -height/2);
    ...
    private getImagePosition(): CropperPosition {
        ...
        const out: CropperPosition = {
            x1: Math.round((this.cropper.x1-50) * ratio),
            y1: Math.round((this.cropper.y1-50) * ratio),
            x2: Math.round((this.cropper.x1+this.cropperStaticWidth-50) * ratio),
            y2: Math.round((this.cropper.y1+this.cropperStaticHeight-50) * ratio)
        };

        if (!this.containWithinAspectRatio) {
            out.x1 = Math.max(out.x1);
            out.y1 = Math.max(out.y1);
