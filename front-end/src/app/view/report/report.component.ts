...
export class ReportComponent implements AfterViewInit, OnInit {
  ...
  applyFilter() {
    ...
    let filteredData= allData.filter(one => {
      ...
      if ((state==0||state == one.state_id) && (minDate <= createdAt && createdAt <= maxDate) &&(((minInstallmentDate <=firstInsDate&&firstInsDate<= maxInstallmentDate)&&(!(one.first_installment== null||one.first_installment== 0)))||((minInstallmentDate <= secondInsDate&& secondInsDate<= maxInstallmentDate)&&(!(one.second_installment== null||one.second_installment== 0)))||((minInstallmentDate <= thirdInsDate&& thirdInsDate<= maxInstallmentDate)&&(!(one.third_installment== null||one.third_installment== 0)))||((minInstallmentDate <= forthInsDate&& forthInsDate<= maxInstallmentDate)&&(!(one.forth_installment== null||one.forth_installment== 0)))) && ((minInv <= parseInt(one.first_ins_invoice) && parseInt(one.first_ins_invoice) <= maxInv)||(minInv <= parseInt(one.second_ins_invoice) && parseInt(one.second_ins_invoice) <= maxInv)||(minInv <= parseInt(one.third_ins_invoice) && parseInt(one.third_ins_invoice) <= maxInv)||(minInv <= parseInt(one.forth_ins_invoice) && parseInt(one.forth_ins_invoice) <= maxInv))) {
        return true;
      }
